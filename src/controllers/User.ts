...
const getUserDataByEmail =  (req: Request, res: Response, next: NextFunction) => {
  const wantdata = 
  {
    User_Email:req.params.Email
  }
  return User.findOne(wantdata).then(user=>res.status(200).json({user})).catch((error)=>res.status(500).json({error}))
}
export default {
  Register,
  Login,
